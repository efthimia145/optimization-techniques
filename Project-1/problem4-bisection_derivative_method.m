...
fplot(f2);
fplot(f3, 'g');
title('Initial Functions to be minimized [Bisection Derivative Method]', 'FontSize', 23);
legend('f_{1}(x)', 'f_{2}(x)', 'f_{3}(x)', 'FontSize', 18);
xlabel('x', 'FontSize', 23);
ylabel('f_{i}(x)', 'FontSize', 23);
grid on

%% Problem 4 - Bisection Derivtive Method
...
for i=1:size(l, 2)
    ...
    [minimum(i, 1), k(i, 1), ak1, bk1, function_calculations(i, 1)] = bisection_derivative_method(f1, x, l(i), a, b);
    [minimum(i, 2), k(i, 2), ak2, bk2, function_calculations(i, 2)] = bisection_derivative_method(f2, x, l(i), a, b);
    [minimum(i, 3), k(i, 3), ak3, bk3, function_calculations(i, 3)] = bisection_derivative_method(f3, x, l(i), a, b);
    ...
    ylabel('a_{k}/ b_{k}', 'FontWeight','bold', 'FontSize', 23)
    legend('a_{k}', 'b_{k}', 'FontSize', 23);
    hold off;
