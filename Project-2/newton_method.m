function [minimum] = newton_method(f, gradf, hessianf, epsilon, starting_point, step_selection)
    ...
    if  f_hessian(1, 1) > 0 && det(f_hessian) > 0
    ...
    if positive_definite
        ...
        if step_selection == 1 % Constant gk
            ...
            hold off;
            saveas(gcf, "figures/Newton-contours-plot-" + gk_calculation_methods(step_selection) + "-" + ...
            ...
            if k > 1
                ...
                xlabel("k");
                ylabel("f(x,y)");
                saveas(gcf, "figures/Newton-k-plot-" + gk_calculation_methods(step_selection) + "-" +...
                             num2str(starting_point(1))+ num2str(starting_point(2)) + ".pdf");
                ...
                colorbar
                hold off;
                saveas(gcf, "figures/Newton-3d-plot-" + gk_calculation_methods(step_selection) + "-" +...
                             num2str(starting_point(1))+ num2str(starting_point(2)) + ".pdf");
            end
        end
        ...
        if step_selection == 2 % Minimization of f(xk + gk*dk)
            ...
            while norm(f_grad) > epsilon
                ...
                % Find gk to minimize f(xk+gk*dk) using golden section
                % method
                l = 0.01;
                l_limit = 0;
                u_limit = 5;
            ...
            if k > 1
                ...
                z = (x.^3).*exp(-(x.^2)-(y.^2));
                surf(x,y,z);
                title([gk_calculation_methods(step_selection) "Starting Point = ("+starting_point(1)+","+starting_point(2)+")"]);
                xlabel("x");
                ylabel("y");
                zlabel("f(x,y)")
        ...
        fprintf("\nFunction is not positive definite! Try another method.\n")
        minimum = 0;
    end 
end